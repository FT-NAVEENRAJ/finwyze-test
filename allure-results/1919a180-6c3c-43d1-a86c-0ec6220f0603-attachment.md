...
```
Error: locator.textContent: Error: strict mode violation: locator('//span[@id="Active"][text()="Active"]') resolved to 2 elements:
    1) <span id="Active" _ngcontent-ng-c4011930937="" class="badge completed ng-star-inserted">Active</span> aka getByRole('row', { name: 'FTAMCRM 9798 FT.IPRU.AMCRM01@' }).locator('#Active')
    2) <span id="Active" _ngcontent-ng-c4011930937="" class="badge completed ng-star-inserted">Active</span> aka getByRole('row', { name: 'FTIPRUSRREVIEWER' }).locator('#Active')
...
```ts
   ...
   50 |     await page2.getByRole('searchbox', { name: 'Search' }).fill('NAVEENRAJ@FINTUPLE.COM');
>  51 |     const active = await page2.locator('//span[@id="Active"][text()="Active"]').textContent();
   ...
   82 |     await page2.getByRole('searchbox', { name: 'Search' }).fill('NAVEENRAJ@FINTUPLE.COM');
   83 |
