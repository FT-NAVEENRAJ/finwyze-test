...
Call log:
  - navigating to "https://cdi-r3.finwyze.com/", waiting until "load"
    ...
    at C:\Testing Details\Naveenraj\SampleTest\PlaywrightAutomation\tests\01-lam.spec.js:441:16
```
...
> 441 |     await page.goto('https://cdi-r3.finwyze.com');
  ...
  520 | //     await page.getByText('LAM ID Maker', { exact: true }).click();
  521 | //     await new Promise(resolve => setTimeout(resolve, 2000));
  522 | //     await page.locator('//button[@id="internal_user_submit"]').click();
  523 | //     await new Promise(resolve => setTimeout(resolve, 2000));
  524 |        
  525 | //        const [popupPage] = await Promise.all([
  526 | //         page.waitForEvent('popup'),
  527 | //         page.click('button#open-popup')
  528 | //       ]);
  529 | //       const successToast = popupPage.locator('//div[@role="alert" and contains(text(), "Task Created Successfully")]');
  530 | //       const errorToast = popupPage.locator('//div[contains(@class, "toast-title") and contains(text(), "Task is already exist in checker queue")]');
  531 | //       const toastType = await Promise.race([
  532 | //         successToast.waitFor({ state: 'visible', timeout: 5000 }).then(() => 'success'),
  533 | //         errorToast.waitFor({ state: 'visible', timeout: 5000 }).then(() => 'error')
  534 | //       ]);
  535 | //     switch (toastType) {
  536 | //       case 'success':
  537 | //         console.log('Task Created Successfully!');
  538 | //         break;
  539 | //       case 'error':
  540 | //         console.log('Task already exists in checker queue.');
  541 | //         break;
```