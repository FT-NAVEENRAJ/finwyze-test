...
- Location: C:\Testing Details\Naveenraj\SampleTest\PlaywrightAutomation\tests\04-amcManager.spec.js:78:1
...
Call log:
  - waiting for getByRole('alert')
    ...
    at C:\Testing Details\Naveenraj\SampleTest\PlaywrightAutomation\tests\04-amcManager.spec.js:99:58
```
...
```ts
   ...
   76 | });
   77 |
   78 | test("TC_05: Activate an Inactive User and Validate Success Toast", async () => {
...
>  99 |     const activateToast = await page2.getByRole('alert').textContent();
  ...
  114 |
  115 |
  116 | test("TC_06: Upload Stamp Paper using Offline Document Flow", async({}) =>{
  ...
  121 |    });
  122 |
  123 | test("TC_07: Validate Stamp Paper Upload in Offline Document ", async () => {
