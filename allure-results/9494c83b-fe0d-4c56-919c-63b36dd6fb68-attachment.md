...
>  19 |     await page.goto("https://cd-r3.finwyze.com");
   ...
   67 | await contentFrame.locator('#Full_name_prefix').first().selectOption('MR');
   68 | await contentFrame.getByText('Prefix *MSMRMRS').nth(2).click();
   69 |
   70 | // Click on the first input element for Maiden Name Prefix
   71 | await contentFrame.locator('.ft-input-edit > .group > .col-md-12').first().click();
   72 | await contentFrame.locator('#MMaiden_Name_prefix').selectOption('MR');
   73 |
   74 | // Fill Mother's Name
   75 | await contentFrame.locator('div').filter({ hasText: /^Mother\'s Name \*Mother\'s Name is required$/ }).getByRole('textbox').click();
   76 | await contentFrame.locator('div').filter({ hasText: /^Mother\'s Name \*Mother\'s Name is required$/ }).getByRole('textbox').fill('MAHa');
   77 |
   78 | // Fill Father's Name and Relationship
   79 | await contentFrame.locator('#FatherName_prefix').selectOption('MR');
   80 | await contentFrame.locator('#Father_Spouse_relationship').selectOption('FATHER');
   81 |
   82 | // Fill Place of Birth
   83 | await contentFrame.locator('#Place_of_birth').click();
   84 | await contentFrame.locator('#Place_of_birth').fill('CHENNAi');
   85 |
   86 | // Check Residential Address
   87 | await contentFrame.locator('#Residential').check();
   88 |
   89 | // Fill and select District for Residential Address
   90 | await contentFrame.locator('#cdistrict').click();
   91 | await contentFrame.locator('#cdistrict').fill('CHENNAi');
   92 |
   93 | // Check Permanent Address
   94 | await contentFrame.locator('#pResidential').first().check();
   95 |
   96 | // Fill and select District for Permanent Address
   97 | await contentFrame.locator('#pdistrict').click();
   98 | await contentFrame.locator('#pdistrict').fill('CHENNAi');
   99 |
  100 | // Click on the Proceed button
  101 | await contentFrame.getByRole('button', { name: 'Proceed' }).click();
