...
- Name: TC_05: Active an Deactivate User and Validate Success Toast
...
Call log:
  - waiting for getByRole('alert')
    ...
    at C:\Testing Details\Naveenraj\SampleTest\PlaywrightAutomation\tests\04-amcManager.spec.js:107:60
```
...
```ts
   ...
   81 | });
   82 |
   83 | test("TC_05: Active an Deactivate User and Validate Success Toast", async () => {
   ...
   95 |     } else {
   96 |         console.log(`Unexpected status: ${inActive}`);
   97 |     }
   98 |
   99 |     // Deactivate the user
  100 |     await page2.getByText('Active').click();
  ...
  105 |
  106 |     // 🛠️ Correct: Wait and capture toast properly
> 107 |     const deactivateToast = await page2.getByRole('alert').textContent();
      |                                                            ^ Error: locator.textContent: Error: strict mode violation: getByRole('alert') resolved to 2 elements:
  108 |     console.log("Toast Message:", deactivateToast?.trim());
  109 |     await expect(page2.getByRole('alert')).toHaveText('User activated Successfully');
  110 |
  111 |     // Confirm the status has changed to Inactive
  112 |     const inactive = await page2.locator('//span[@id="Active"][text()="Active"]').textContent();
  113 |     if (inactive && inactive.trim() === "InActive") {
  114 |         console.log("Status is correctly set to 'Active'.");
  115 |     } else {
  116 |         console.log(`Unexpected status after deactivation: ${inactive}`);
  117 |     }
  118 | });
  119 |
  120 |
  121 |
  122 | test("TC_06: Upload Stamp Paper using Offline Document Flow", async({}) =>{
  123 |     await new Promise(resolve => setTimeout(resolve, 5000));
  124 |     await page2.locator('//button[text()="Offline Documents"]').click();
  125 |     
  126 |
  127 |    });
  128 |
  129 | test("TC_07: Validate Stamp Paper Upload in Offline Document ", async () => {
  130 |     const addStamp = new OfflineDocumentSection(page2);
  131 |     await addStamp.stampPaperAddition("2","21-10-2025","MAHARASHTRA","PMS AGREEMENT","700");
  132 |     await addStamp.stampPaperAddition("2","21-11-2025","MAHARASHTRA","POA","500");
  133 |
  134 |
  135 | });
  136 |
