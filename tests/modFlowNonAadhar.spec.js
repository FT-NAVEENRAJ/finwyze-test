...
let page, page2;
test.describe('Modification Non-Adhar Flow', () => {
...
await new Promise(resolve => setTimeout(resolve, 1000));
await dialogFrame.locator('(//input[@id="pResidential"])[1]').check();   // - issue
await dialogFrame.locator('//input[@id="pdistrict"][@formcontrolname="pdistrict"]').fill("Chennai");
...
await page2.waitForTimeout(2000); 
  ...
  test("TC_16: Verify the Confirmation Popup of Application Mode", async()=>{
    await new Promise(resolve => setTimeout(resolve, 1000));
    await page2.getByRole('dialog').locator('iframe').contentFrame().getByRole('radio').nth(1).check();
    await page2.getByRole('dialog').locator('iframe').contentFrame().getByRole('button', { name: 'Proceed' }).click();
    await new Promise(resolve => setTimeout(resolve, 2000));
    if (await page2.getByRole('button', { name: 'Proceed' }).isVisible()) {
      await new Promise(resolve => setTimeout(resolve, 2000));
      await page2.getByRole('button', { name: 'Proceed' }).click();
      await page2.waitForTimeout(500); // wait after click
      ...
      console.log('"Proceed" button not visible. Skipping steps.');
    }
