...
test.beforeAll(async ({ browser }) => {
    ...
    await lamMaker.loginLamMaker('cust.appl.mkr@fintuple.com', 'Icici@123');
    
  });
  test("TC_01: Login to iCACE Custody Maker with valid credentials", async () => {
  ...
  test("TC_03: To review all PDF documents and verify the AML check is successful, then approve the task", async()=>{
      ...
      await Maker.custodyMakerApprove();
      await new Promise(resolve => setTimeout(resolve, 3000));
